...
function ana = apc_restrict(rate,L_p,Pd,Per)
    ...
    fun_temp = fsolve(@p_tau_AP,[0 0.05 0],optimset('Display','off'));
    q           = fun_temp(1);
    tau            = fun_temp(2);
    pw            = fun_temp(3);
    %{
    ...
    Ptr         = 1 - Ptr_temp;
    Ps_temp     = (1-tau)*(1-tau*(1-tau)^(loc_nodeNum-1))*(1 - tau)^(loc_nodeNum-1);
